...
// Sub-components
// ---------------------------------------------------------------------------
function OscLog({ messages }: { messages: OscMessage[] }) {
  return (
    <div className="flex flex-col gap-1 max-h-[120px] overflow-y-auto
      scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent pr-0.5">
      <AnimatePresence initial={false}>
        {messages.map(m => (
          ...
          >
            <span className="text-primary/80 shrink-0">{m.address}</span>
            <span className="text-accent">{m.value}</span>
            <span className="text-muted-foreground/40 text-[9px]">{m.type}</span>
          </motion.div>
        ))}
      </AnimatePresence>
      {messages.length === 0 && (
        <p className="font-mono text-[10px] text-muted-foreground/40 italic">
          Move a slider to see outgoing OSC messages…
        </p>
      )}
    </div>
  );
...
function BandSlider({ bandKey, sym, label, hue, value, onChange, onCommit }: BandSliderProps) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span className="w-4 text-center font-mono text-[12px] shrink-0"
      ...
      <span className="w-8 text-right font-mono text-[10px] shrink-0"
        style={{ color }}>
        {value.toFixed(2)}
      </span>
    </div>
...
export default function ManualModePanel({
  isOpen, onClose, emotion, bands, oscMessages,
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4
            ...
            <div>
              <p className="text-[10px] font-semibold tracking-[0.3em]
                text-accent/80 uppercase">Manual Override</p>
            ...
            <button
              onClick={onClose}
              className="p-1.5 rounded-lg border border-border/30
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-5 px-5 py-4
            ...
            <div className="flex flex-col gap-3">
              <p className="text-[9px] font-semibold tracking-[0.3em]
                text-muted-foreground uppercase">Emotion Preset</p>
            ...
            <div className="flex flex-col gap-3">
              <p className="text-[9px] font-semibold tracking-[0.3em]
                text-muted-foreground uppercase">EEG Band Power</p>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <p className="text-[9px] font-semibold tracking-[0.3em]
                  text-muted-foreground uppercase">Outgoing OSC</p>
                <div className="flex items-center gap-1.5">
                  ...
                  />
                  <span className="font-mono text-[9px] text-accent/70">LIVE</span>
                </div>
              </div>
              <div className="rounded-xl border border-border/30
                bg-black/30 p-3">
                <OscLog messages={oscMessages} />
              </div>
            </div>
