...
const MonitoringScreen = ({ onPatternReady }: Props) => {
  ...
  const alpha = brainData?.alpha ?? 0.72;
  const beta = brainData?.beta ?? 0.45;
  const confidence = brainData?.confidence ?? 0;
  const gamma = brainData?.gamma ?? 0.31;
  const theta = brainData?.theta ?? 0.09;
  const signal = brainData?.signal_quality ?? 94;
  const isConnected = Boolean(calibration && brainData);
